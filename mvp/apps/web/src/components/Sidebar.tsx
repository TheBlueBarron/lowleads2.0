  ...
  return (
    <aside className="w-64 shrink-0 bg-gray-900 flex flex-col h-screen">
      ...
      <div className="px-6 py-5 border-b border-gray-800">
        <span className="text-white font-bold text-lg tracking-tight">Lowleads</span>
        <p className="text-gray-400 text-xs mt-0.5">B2B Referral Marketplace</p>
      </div>
        ...
        {user && (
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-semibold shrink-0">
              {user.role === 'company_owner' ? 'O' : 'T'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-white truncate">
                {user.role === 'company_owner' ? 'Company Owner' : 'Technician'}
              </p>
              <p className="text-xs text-gray-400 truncate">{user.sub.slice(0, 8)}…</p>
            </div>
          </div>
        )}
        <Button
